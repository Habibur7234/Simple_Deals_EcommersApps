...
@Dao
public interface CartDAO { //DAO = Data Access Object
    ...
    @Query("SELECT SUM((foodPrice+foodExtraPrice) * foodQuantity)+shippingFee FROM Cart WHERE uid=:uid")
    ...
    @Query("SELECT * FROM Cart WHERE categoryId=:categoryId AND uid=:uid AND foodId=:foodId AND foodSize=:foodSize AND foodAddon =:foodAddon")
